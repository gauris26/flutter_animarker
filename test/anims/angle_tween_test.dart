...
void main() {
  group('Angle Tween Interpolation', () {
    test('''Transform should return begin angle at 0.0 position (t) on timeline,
      ...
      useShortestAngle.listen(
        expectAsync1<void, bool>(
          (value) {
            var interpolator =
                AngleInterpolatorImpl(begin: beginAngle, end: endAngle, findShortestAngle: value);
    ...
    test('Transform should return end shortest angle at 1.0 position (t) on timeline', () {
      var useShortestAngle = Stream<bool>.fromIterable([true, false]);
      var beginAngle = 152.0;
      var endAngle = 345.0;
      var t = 1.0;
        ...
        expectAsync1<void, bool>(
          (value) {
            var interpolator =
                AngleInterpolatorImpl(begin: beginAngle, end: endAngle, findShortestAngle: value);
            var angleTween = BearingTween(interpolator: interpolator);
            var shortestAngle = value ? ((endAngle - beginAngle) + 180) % 360 - 180 : endAngle;
    ...
    //
    test('''lerp(t) should return counterclockwise angles if the delta (end-begin) is
    ...
    test('''lerp(t) should return zero when trying to interpolate between angles
      ...
      useShortestAngle.listen(
        expectAsync1<void, bool>(
          (value) {
            var interpolator = AngleInterpolatorImpl(begin: beginAngle, end: endAngle, findShortestAngle: value);
            var angleTween = BearingTween(interpolator: interpolator);
            var result = angleTween.lerp(t);
